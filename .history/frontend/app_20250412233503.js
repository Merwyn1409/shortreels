...
async function checkRequestStatus(requestId) {
  try {
    ...
    console.log('Status check data:', data); // Debug logging
...
async function showWatermarkedPreview(videoUrl) {
  try {
    ...
    // Debug logging
    console.log('Showing video preview from URL:', videoUrl);
    
    // Construct proper URL - handle both relative and absolute URLs
    let absoluteUrl;
    if (videoUrl.startsWith('http')) {
      absoluteUrl = videoUrl;
    } else if (videoUrl.startsWith('/')) {
      absoluteUrl = `http://localhost:8000${videoUrl}`;
    } else {
      absoluteUrl = `http://localhost:8000/serve-video/${currentRequestId}?watermarked=true`;
    }

    // Add cache busting parameter
    absoluteUrl += `${absoluteUrl.includes('?') ? '&' : '?'}t=${Date.now()}`;
    
    console.log('Final video URL:', absoluteUrl);
    ...
    return new Promise((resolve, reject) => {
      ...
      videoPreview.onerror = () => {
        console.error('Video load error:', videoPreview.error);
